...
describe Forking::Handler do
  ...
  before :each do
    ...
    allow(repo).to receive_messages(uri: 'foo@example.com:bar/dir',
                                    directory: 'dir',
                                    domain: 'example.com')
  ...
  context 'standard forking' do
    let(:handler) { described_class.new(repository: repo) }
  ...
  context 'skipping prs' do
    let(:handler) { described_class.new(repository: repo, download_pull_request: false) }
    it 'does not add the prs' do
      expect(handler).to receive(:system)
        .with('git config --add remote.upstream.fetch "+refs/pull/*/head:refs/remotes/upstream/pr/*"').never
      expect(handler).to receive(:system)
        .with('git config --add remote.origin.fetch "+refs/pull/*/head:refs/remotes/origin/pr/*"').never
      handler.handle
    end
  end

  context 'indicating a directroy' do
    let(:handler) { described_class.new(repository: repo, directory: 'foo') }
  ...
  context 'YAML is not working' do
    ...
    before(:each) do
      allow(YAML).to receive(:load_file) { fail LoadError }
      allow(handler).to receive(:system)
    end
